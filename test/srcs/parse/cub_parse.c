...
/*   Updated: 2021/05/21 20:53:03 by soooh            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			cub_check_arg(int argc, char *argv[], int *fd)
{
	if (argc != 2 && !(argc == 3 && !cub_strcmp("--save", argv[2])))
		print_ele_error(INVALID_INPUT_VALUE);
...
int				parse(t_par *par, int fd)
{
	...
	while (get_next_line(fd, &line) > 0 && par->ele.total == 8)
		parsing_m(line, &par->map);
	if (par->map.map_dir == 0)
		print_ele_error(NO_PLAYER);
	free(line);
	close(fd);
	cub_map_missing(par);
